...
pub fn build_decision(
    ...
    let (action, reason) = decide_action(peak, scored.average, policy, &scored.categories);
...
mod tests {
    ...
    #[test]
    fn bans_on_peak() {
        let (action, _) = decide_action(95, 40, &policy(), &["MODULE".into()]);
        assert_eq!(action, DecisionAction::Ban);
    }

    #[test]
    fn kicks_on_sustained() {
        let (action, _) = decide_action(75, 55, &policy(), &["HANDLE".into()]);
    ...
    fn observes_below_threshold() {
        let (action, _) = decide_action(30, 20, &policy(), &["OVERLAY".into()]);
    ...
    fn rolls_session_peak_forward() {
        ...
        let decision = build_decision(&session, &[event("HOOK", 50)], &policy());
